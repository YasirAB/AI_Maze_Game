...
public class SeekerEnemyTest {
	...
	public void testSeekMoveValid(){
		...
		p.seekMove(20,15);
		pathToUse = myPath.findPath(p.getRow(), p.getCol(), 20, 15);
	...
	public void testSeekMoveInvalid(){
		...
		ArrayList<MapNode> old = pathToUse;
		p.seekMove(11,11);
		pathToUse = myPath.findPath(p.getRow(), p.getCol(), 11, 11);
	...
	@Test
	public void testTakeTurnDoubleSeek(){
		Board b = new Board(30,30);
		SeekerEnemy p = new SeekerEnemy(b,0, 9, 10);
		p.setSpeed(-10);
		p.setFocus(100);
		...
		p.takeTurn(20,15);
		pathToUse = myPath.findPath(p.getRow(), p.getCol(), 20, 15);
		assertTrue("testing is size of new path less than old path", pathToUse.size() - old.size() == -2);
