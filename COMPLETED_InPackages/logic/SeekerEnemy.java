...
public class SeekerEnemy extends Piece{
  ...
  */
  ...
  public SeekerEnemy(Board board, double level, int startRow, int startCol) {
    ...
    maze.board[getRow()][getCol()].setEnemy(true);
  ...
  public void moveToTile(int nextRow, int nextCol) {
    if (nextRow == getRow()+1 && !maze.board[nextRow][nextCol].checkEnemy()) {
      maze.board[getRow()][getCol()].setEnemy(false);
      moveDown();
      maze.board[getRow()][getCol()].setEnemy(true);
    } else if (nextRow == getRow()-1 && !maze.board[nextRow][nextCol].checkEnemy()) {
      maze.board[getRow()][getCol()].setEnemy(false);
      moveUp();
      maze.board[getRow()][getCol()].setEnemy(true);
    } else if (nextCol == getCol()+1 && !maze.board[nextRow][nextCol].checkEnemy()) {
      maze.board[getRow()][getCol()].setEnemy(false);
      moveRight();
      maze.board[getRow()][getCol()].setEnemy(true);
    } else if (nextCol == getCol()-1 && !maze.board[nextRow][nextCol].checkEnemy()) {
  ...
  */
  public void seekMove(int playerRow, int playerColumn) {
    ArrayList<MapNode> pathToUse = myPath.findPath(getRow(), getCol(), playerRow, playerColumn);
    if (pathToUse.size() > 0) {
      MapNode current = pathToUse.get(0);
      if (!current.checkEnemy()) {
        moveToTile(current.getRow(), current.getCol());
      } else {
        randomMove();
      }
    } else {
 ...
 */
  public void randomMove() {
    ArrayList<int[]> directions = surch.checkAround(maze, getRow(), getCol());
